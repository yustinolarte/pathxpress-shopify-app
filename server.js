...
app.get("/app", requireSessionToken, async (req, res) => {
                ...
                shipmentsRows = rows.map(row => {
                    ...
                    <tr id="order-row-${row.id}" data-type="${typeKey}">
                        ...
                        <td style="display: flex; gap: 8px; align-items: center;">
                            ${row.waybillNumber
                            ? `<button class="print-btn" onclick='printWaybill(${shipmentData})'><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-right:4px;vertical-align:middle;"><polyline points="6 9 6 2 18 2 18 9"></polyline><path d="M6 18H4a2 2 0 0 1-2-2v-5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v5a2 2 0 0 1-2 2h-2"></path><rect x="6" y="14" width="12" height="8"></rect></svg>Print Label</button>
                               <a href="/app/orders/${encodeURIComponent(row.waybillNumber)}/tracking?shop=${encodeURIComponent(shop)}" class="btn-sm" style="text-decoration:none;"><svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" style="margin-right:2px;vertical-align:middle;"><circle cx="12" cy="12" r="9"/><path d="M12 7v5l3 2"/></svg>Track</a>`
    ...
    <html>
      <head>
        ...
        <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&family=Hanken+Grotesk:wght@400;500;600;700&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
  ...
  *,*::before,*::after{box-sizing:border-box}
  ...
  :root{
    ...
    --ink:#16181C;--ink-2:#3C4046;--muted:#6B6F77;
    --line:rgba(20,22,26,.11);--line-strong:rgba(20,22,26,.20);
    --accent:#E10600;--accent-ink:#FFFFFF;
    --band:#16181C;--band-ink:#F4F3EF;
    --shadow-sm:0 1px 2px rgba(20,22,26,.05);
    --shadow:0 2px 6px rgba(20,22,26,.05),0 18px 40px -24px rgba(20,22,26,.22);
    --st-green:#1F8A5B;--st-green-bg:#1f8a5b1a;
    --st-amber:#B5790A;--st-amber-bg:#b5790a1a;
    --st-blue:#2A6FDB;--st-blue-bg:#2a6fdb1a;
  }
  @keyframes pxpulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.3)}}
  body{font-family:var(--font-body);background:var(--bg);color:var(--ink);-webkit-font-smoothing:antialiased;min-height:100vh;padding-bottom:60px;}
  .app-header{display:flex;align-items:center;gap:18px;padding:16px 32px;background:var(--surface);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:20;}
  .app-header-logo{display:flex;align-items:center;text-decoration:none;flex:none;}
  .app-header-logo-text{font-family:var(--font-display);font-weight:800;font-size:18px;letter-spacing:-.03em;color:var(--ink);}
  .app-header-logo-text em{color:var(--accent);font-style:normal;}
  .app-header-divider{width:1px;height:26px;background:var(--line);flex:none;}
  .app-header-shop{display:flex;flex-direction:column;line-height:1.25;}
  .app-header-shop-label{font-family:var(--font-mono);font-size:10px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);}
  .app-header-shop-name{font-family:var(--font-display);font-weight:600;font-size:14px;}
  .app-header-right{margin-left:auto;display:flex;align-items:center;gap:14px;}
  .badge-connected{display:inline-flex;align-items:center;gap:8px;font-family:var(--font-display);font-weight:600;font-size:13px;color:var(--st-green);padding:7px 14px;border-radius:var(--radius-pill);background:var(--st-green-bg);}
  .badge-connected-dot{width:8px;height:8px;border-radius:50%;background:var(--st-green);animation:pxpulse 2s infinite;}
  .badge-disconnected{display:inline-flex;align-items:center;gap:8px;font-family:var(--font-display);font-weight:600;font-size:13px;color:var(--accent);padding:7px 14px;border-radius:var(--radius-pill);background:color-mix(in srgb,var(--accent) 12%,transparent);}
  .badge-disconnected-dot{width:8px;height:8px;border-radius:50%;background:var(--accent);}
  .app-nav-wrap{padding:18px 32px 0;max-width:1180px;margin:0 auto;}
  .app-nav{display:flex;gap:4px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius-sm);padding:6px;width:max-content;max-width:100%;box-shadow:var(--shadow-sm);}
  .app-nav a{display:inline-flex;align-items:center;gap:8px;padding:11px 16px;border-radius:9px;font-family:var(--font-body);font-size:14px;font-weight:600;cursor:pointer;border:none;background:none;white-space:nowrap;text-decoration:none;color:var(--muted);transition:color .15s;}
  .app-nav a:hover{color:var(--ink);}
  .app-nav a.active{background:color-mix(in srgb,var(--accent) 12%,transparent);color:var(--accent);}
  .app-nav a svg{width:17px;height:17px;flex-shrink:0;}
  .nav-badge{font-family:var(--font-mono);font-size:10px;background:var(--accent);color:#fff;border-radius:20px;padding:1px 7px;font-weight:700;}
  .app-main{max-width:1180px;margin:0 auto;padding:26px 32px 0;}
  .page-eyebrow{font-family:var(--font-mono);font-size:11px;letter-spacing:.14em;text-transform:uppercase;color:var(--muted);display:inline-flex;align-items:center;gap:10px;margin-bottom:12px;}
  .page-eyebrow-line{width:22px;height:1px;background:var(--accent);}
  h1.page-title{font-family:var(--font-display);font-weight:600;font-size:30px;letter-spacing:-.02em;margin:0 0 4px;}
  ...
  .card-nop{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow-sm);margin-bottom:18px;overflow:hidden;}
  ...
  .kpi-card.band{position:relative;overflow:hidden;background:var(--band);color:var(--band-ink);border-color:transparent;}
  .kpi-top{display:flex;align-items:center;justify-content:space-between;}
  .kpi-label{font-size:13px;color:var(--muted);font-weight:500;}
  .kpi-card.band .kpi-label{color:color-mix(in srgb,var(--band-ink) 65%,transparent);}
  .kpi-icon{width:34px;height:34px;border-radius:9px;display:grid;place-items:center;background:var(--surface-2);color:var(--ink);}
  .kpi-card.band .kpi-icon{background:color-mix(in srgb,var(--band-ink) 12%,transparent);color:var(--band-ink);}
  .kpi-val{font-family:var(--font-display);font-weight:600;font-size:34px;letter-spacing:-.03em;margin:14px 0 4px;}
  .kpi-sub{font-size:12.5px;color:var(--muted);}
  .kpi-card.band .kpi-sub{color:color-mix(in srgb,var(--band-ink) 65%,transparent);}
  .card-header{display:flex;align-items:center;justify-content:space-between;padding:18px 22px;border-bottom:1px solid var(--line);gap:14px;flex-wrap:wrap;}
  .card-header-title{font-family:var(--font-display);font-weight:600;font-size:16.5px;margin:0;}
  .card-header-sub{font-size:13px;color:var(--muted);margin-top:2px;}
  .table-wrap{overflow-x:auto;padding:8px 8px 6px;}
  table{width:100%;border-collapse:collapse;font-size:13.5px;}
  thead th{text-align:left;font-family:var(--font-mono);font-size:10.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);font-weight:400;padding:8px 14px 12px;border-bottom:1px solid var(--line);white-space:nowrap;}
  thead th.th-right{text-align:right;}
  tbody td{padding:14px;border-bottom:1px solid var(--line);}
  tbody tr:last-child td{border-bottom:none;}
  .badge{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;font-family:var(--font-display);white-space:nowrap;}
  .badge-dot{width:6px;height:6px;border-radius:50%;background:currentColor;}
  .badge.green{color:var(--st-green);background:var(--st-green-bg);}
  .badge.amber{color:var(--st-amber);background:var(--st-amber-bg);}
  .badge.blue{color:var(--st-blue);background:var(--st-blue-bg);}
  .badge.gray{color:var(--muted);background:color-mix(in srgb,var(--muted) 14%,transparent);}
  .badge.red{color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,transparent);}
  .btn-primary{display:inline-flex;align-items:center;gap:8px;height:44px;padding:0 20px;border-radius:var(--radius-pill);font-family:var(--font-display);font-weight:600;font-size:14px;border:none;cursor:pointer;background:var(--accent);color:var(--accent-ink);}
  .btn-primary:disabled{background:var(--surface-2);color:var(--muted);border:1px solid var(--line);cursor:not-allowed;}
  .btn-primary-lg{height:50px;padding:0 28px;font-size:15.5px;}
  .btn-outline{display:inline-flex;align-items:center;gap:8px;height:44px;padding:0 20px;border-radius:var(--radius-pill);font-family:var(--font-display);font-weight:600;font-size:14px;border:1px solid var(--line-strong);background:transparent;color:var(--ink);cursor:pointer;}
  .btn-sm{display:inline-flex;align-items:center;gap:6px;padding:7px 13px;border-radius:8px;border:1px solid var(--line-strong);background:transparent;color:var(--ink);font-family:var(--font-display);font-weight:600;font-size:12.5px;cursor:pointer;}
  .btn-sm:hover{background:var(--surface-2);}
  .toolbar{display:flex;align-items:center;gap:14px;flex-wrap:wrap;padding:14px 18px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius-sm);margin-bottom:16px;box-shadow:var(--shadow-sm);}
  .search-wrap{display:flex;align-items:center;gap:8px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius-pill);padding:8px 16px;color:var(--muted);}
  .search-wrap input{border:none;background:none;outline:none;font-family:var(--font-body);font-size:13.5px;color:var(--ink);width:100%;}
  label.field-label{display:block;font-family:var(--font-display);font-weight:600;font-size:13.5px;margin-bottom:8px;}
  input[type="text"].field,input[type="number"].field{width:100%;height:44px;padding:0 14px;border-radius:var(--radius-sm);border:1px solid var(--line-strong);background:var(--surface-2);color:var(--ink);font-family:var(--font-body);font-size:14px;outline:none;}
  input[type="text"].field:focus,input[type="number"].field:focus{border-color:var(--accent);box-shadow:0 0 0 3px color-mix(in srgb,var(--accent) 15%,transparent);}
  select.field{width:100%;height:44px;padding:0 14px;border-radius:var(--radius-sm);border:1px solid var(--line-strong);background:var(--surface-2);color:var(--ink);font-family:var(--font-body);font-size:14px;outline:none;cursor:pointer;}
  p.field-help{font-size:12.5px;color:var(--muted);margin:10px 0 0;}
  .feedback-box{padding:12px 16px;border-radius:var(--radius-sm);font-size:14px;display:none;margin-bottom:16px;}
  .feedback-success{background:var(--st-green-bg);border:1px solid color-mix(in srgb,var(--st-green) 30%,transparent);color:var(--st-green);}
  .feedback-error{background:color-mix(in srgb,var(--accent) 10%,transparent);border:1px solid color-mix(in srgb,var(--accent) 30%,transparent);color:var(--accent);}
  .feedback-info{background:var(--st-blue-bg);border:1px solid color-mix(in srgb,var(--st-blue) 30%,transparent);color:var(--st-blue);}
  .sync-option{display:flex;align-items:flex-start;gap:12px;padding:14px 16px;border-radius:var(--radius-sm);cursor:pointer;border:1px solid var(--line);background:var(--surface-2);margin-bottom:10px;}
  .sync-option.selected{border-color:var(--accent);background:color-mix(in srgb,var(--accent) 8%,transparent);}
  .radio-outer{flex:none;width:18px;height:18px;margin-top:1px;border-radius:50%;display:grid;place-items:center;border:2px solid var(--line-strong);}
  .sync-option.selected .radio-outer{border-color:var(--accent);}
  .radio-dot{width:9px;height:9px;border-radius:50%;background:var(--accent);transform:scale(0);transition:transform .15s;}
  .sync-option.selected .radio-dot{transform:scale(1);}
  .section-heading{display:flex;align-items:center;gap:9px;font-family:var(--font-display);font-weight:600;font-size:16px;margin-bottom:16px;}
  .filter-tabs{display:flex;background:var(--surface-2);border:1px solid var(--line);border-radius:9px;padding:4px;gap:4px;}
  .filter-tab{padding:6px 14px;font-size:12.5px;font-weight:600;font-family:var(--font-display);border-radius:6px;color:var(--muted);cursor:pointer;border:none;background:none;}
  .filter-tab.active{background:var(--accent);color:#fff;}
  .banner-blue{display:flex;align-items:center;gap:14px;padding:14px 18px;border-radius:var(--radius);border:1px solid color-mix(in srgb,var(--st-blue) 30%,transparent);background:var(--st-blue-bg);margin-bottom:18px;}
  .banner-red{display:flex;align-items:center;gap:14px;padding:16px 18px;border-radius:var(--radius);border:1px solid color-mix(in srgb,var(--accent) 26%,transparent);background:color-mix(in srgb,var(--accent) 6%,transparent);margin-bottom:18px;}
  .pagination{display:flex;gap:8px;margin-top:14px;justify-content:flex-end;}
  .btn-page{padding:7px 14px;border-radius:8px;border:1px solid var(--line);background:var(--surface);font-size:13px;color:var(--ink);text-decoration:none;cursor:pointer;}
  .btn-page.disabled{color:var(--muted);cursor:default;pointer-events:none;}
  .verified-box{display:flex;align-items:center;gap:8px;margin-top:12px;padding:10px 14px;border-radius:var(--radius-sm);background:var(--st-green-bg);color:var(--st-green);font-size:13px;font-weight:600;font-family:var(--font-display);}
  .grid-2{display:grid;grid-template-columns:1fr 1fr;gap:15px;}
  input[type="checkbox"]{width:17px;height:17px;accent-color:var(--accent);cursor:pointer;}
  .progress-bar{display:none;height:3px;background:var(--line);border-radius:2px;margin-bottom:12px;overflow:hidden;}
  .progress-bar .fill{height:100%;background:var(--accent);transition:width 0.3s;width:0%;}
  @media(max-width:768px){
    .app-header{padding:12px 16px;}
    .app-nav-wrap{padding:12px 16px 0;}
    ...
    .grid-2{grid-template-columns:1fr;}
        ...
        <script>
            ...
            // Prefer the official label generated server-side by the portal (single
            // source of truth). Falls back to the in-browser generator if the portal
            // proxy is unavailable.
            const PX_SHOP = ${JSON.stringify(shop)};
            async function printWaybill(shipment) {
                const wb = shipment && shipment.waybillNumber;
                if (!wb) { return generateWaybillPDF(shipment); }
                try {
                    const resp = await fetch('/app/waybill/' + encodeURIComponent(wb) + '.pdf?shop=' + encodeURIComponent(PX_SHOP));
                    ...
                    console.warn('Official waybill unavailable, using local generator:', e.message);
                    generateWaybillPDF(shipment);
                }
            }
      ...
      <body>
        ...
        <header class="app-header">
          <a href="/app?shop=${shop}" class="app-header-logo"><span class="app-header-logo-text">PATH<em>X</em>PRESS</span></a>
          <div class="app-header-divider"></div>
          <div class="app-header-shop">
            <span class="app-header-shop-label">Shopify App</span>
            <span class="app-header-shop-name">${shop}</span>
          </div>
          <div class="app-header-right">
            ${isConnected ? '<span class="badge-connected"><span class="badge-connected-dot"></span>Connected</span>' : '<span class="badge-disconnected"><span class="badge-disconnected-dot"></span>Not connected</span>'}
          </div>
        </header>
        ...
        <div class="app-nav-wrap">
          <nav class="app-nav">
            <a href="/app?shop=${shop}" class="active">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="7" height="9" rx="1.5"/><rect x="14" y="3" width="7" height="5" rx="1.5"/><rect x="14" y="12" width="7" height="9" rx="1.5"/><rect x="3" y="16" width="7" height="5" rx="1.5"/></svg>
              Dashboard
            </a>
            <a href="/app/orders?shop=${shop}">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m12 3 8 4.5v9L12 21l-8-4.5v-9L12 3Z"/><path d="m4 7.5 8 4.5 8-4.5M12 21v-9"/></svg>
              Orders
            </a>
            <a href="/app/stuck-orders?shop=${shop}">
              <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
              Rescue
              ${stuckCount > 0 ? `<span class="nav-badge">${stuckCount}</span>` : ''}
            </a>
        ...
        <main class="app-main">
          <div style="margin-bottom:22px;">
            <span class="page-eyebrow"><span class="page-eyebrow-line"></span>Last-mile · EAU</span>
            <h1 class="page-title">Dashboard</h1>
            <p class="page-sub">Your PathXpress shipping activity at a glance.</p>
          </div>

        <div class="card" style="display:none;">
            <div class="header-logo-legacy">
        ...
        ${isConnected
            ? `
              <div class="kpi-grid">
                <div class="kpi-card">
                  <div class="kpi-top">
                    <span class="kpi-label">Shipments today</span>
                    <span class="kpi-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="m12 3 8 4.5v9L12 21l-8-4.5v-9L12 3Z"/></svg></span>
                  </div>
                  <div class="kpi-val">${metrics.todayCount || 0}</div>
                  <div class="kpi-sub">shipments created today</div>
                </div>
                <div class="kpi-card">
                  <div class="kpi-top">
                    <span class="kpi-label">Active shipments</span>
                    <span class="kpi-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 7h11v8H3zM14 10h4l3 3v2h-7z"/><circle cx="6.5" cy="16.5" r="1.5"/><circle cx="17.5" cy="16.5" r="1.5"/></svg></span>
                  </div>
                  <div class="kpi-val">${metrics.activeCount || 0}</div>
                  <div class="kpi-sub">in transit across the network</div>
                </div>
                <a href="/app/cod?shop=${shop}" class="kpi-card band" style="text-decoration:none;display:block;">
                  <span style="position:absolute;right:-18px;bottom:-46px;font-family:var(--font-display);font-weight:700;font-size:150px;line-height:1;color:var(--accent);opacity:.16;pointer-events:none;">X</span>
                  <div class="kpi-top" style="position:relative;">
                    <span class="kpi-label">Pending COD</span>
                    <span class="kpi-icon"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="2.5" y="6" width="19" height="12" rx="2"/><circle cx="12" cy="12" r="2.6"/></svg></span>
                  </div>
                  <div class="kpi-val" style="position:relative;">AED ${(metrics.pendingCod || 0).toLocaleString()}</div>
                  <div class="kpi-sub" style="position:relative;">View reconciliation →</div>
                </a>
              </div>
              ...
              <div style="display:none">
                <form action="/app/save-settings" method="POST" id="settingsForm">
                    ...
                    <div class="settings-section">
                        <select name="default_service_type" id="default_service_type" class="field">
              ...
              <div class="card">
                ...
                <div class="banner-blue">
                  <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--st-blue)" stroke-width="1.8" style="flex:none;"><path d="M18 20V10M12 20V4M6 20v-6"/></svg>
                  <div style="flex:1;"><strong style="font-family:var(--font-display);color:var(--ink);font-size:14px;">Manual sync mode is active.</strong><span style="display:block;font-size:12.5px;color:var(--muted);margin-top:2px;">New orders are not sent automatically. Pick which ones to ship from the Orders page.</span></div>
                  <a href="/app/orders?shop=${shop}" class="btn-primary" style="text-decoration:none;">View orders →</a>
                </div>
                ` : ''}
                <div class="card-header">
                  <div><h3 class="card-header-title">My PathXpress shipments</h3><div class="card-header-sub">Latest movements from your store</div></div>
                  <a href="/app/orders?shop=${shop}" class="btn-primary" style="text-decoration:none;"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M12 5v14M5 12h14"/></svg>Manage orders</a>
                </div>

                <!-- Filter tabs + search -->
                <div style="display:flex;align-items:center;gap:10px;padding:16px 22px 10px;flex-wrap:wrap;">
                  <div class="filter-tabs">
                    <button onclick="filterTable('all')" id="tab-all" class="filter-tab active">All</button>
                    <button onclick="filterTable('shipment')" id="tab-shipment" class="filter-tab">Shipments</button>
                    <button onclick="filterTable('return')" id="tab-return" class="filter-tab">Returns</button>
                    <button onclick="filterTable('exchange')" id="tab-exchange" class="filter-tab">Exchanges</button>
                  </div>
                  <div class="search-wrap" style="flex:1;min-width:180px;max-width:300px;">
                    ...
                    <input type="text" id="shipment-search" onkeyup="searchTable()" placeholder="Search order # or waybill…">
                  </div>
                </div>

                <div class="table-wrap"><table id="shipments-table">
                    <thead>
                        <tr>
                            <th>Order #</th>
                            <th>Waybill</th>
                ...
                <p id="no-results-msg" style="display:none;color:var(--muted);text-align:center;padding:20px;">No shipments found.</p>
              </div></div>
            `
            : `
              <div class="card" style="text-align:center;padding:56px 28px;">
                <div style="width:56px;height:56px;border-radius:15px;display:grid;place-items:center;margin:0 auto 18px;background:color-mix(in srgb,var(--accent) 12%,transparent);color:var(--accent);">
                  <svg width="26" height="26" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M10 13a5 5 0 0 0 7.54.54l3-3a5 5 0 0 0-7.07-7.07l-1.72 1.71"/><path d="M14 11a5 5 0 0 0-7.54-.54l-3 3a5 5 0 0 0 7.07 7.07l1.71-1.71"/></svg>
                </div>
                <h2 style="font-family:var(--font-display);font-weight:600;font-size:24px;margin:0 0 8px;color:var(--ink);">Connect your store</h2>
                <p style="color:var(--muted);font-size:15px;max-width:360px;margin:0 auto 26px;">Link your Shopify store to PathXpress to start creating shipments, printing labels and reconciling COD.</p>
                <a href="/auth?shop=${shop}" target="_top" class="btn-primary btn-primary-lg" style="text-decoration:none;">
                  <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M4.5 16.5c-1.5 1.26-2 5-2 5s3.74-.5 5-2c.71-.84.7-2.13-.09-2.91a2.18 2.18 0 0 0-2.91-.09zM12 15l-3-3a22 22 0 0 1 2-3.95A12.88 12.88 0 0 1 22 2c0 2.72-.78 7.5-6 11a22.35 22.35 0 0 1-4 2z"/></svg>
                  Connect now
                </a>
              </div>
            `
...
app.get("/app/settings", requireSessionToken, async (req, res) => {
    ...
    // global default can't carry the per-order date/time they require.
    ...
    try {
        ...
        if (usable.length) {
            serviceOptionsHtml = usable.map(s =>
                `<option value="${escHtml(s.code)}" ${currentServiceType === s.code ? 'selected' : ''}>${escHtml(s.displayName)}${s.deliveryTime ? ` — ${escHtml(s.deliveryTime)}` : ''}</option>`
            ).join("\n                ");
        }
    } catch(_e) {}
    if (!serviceOptionsHtml) {
        const fallback = [
            { code: 'DOM', label: 'DOM — Next Day (1-2 days)' },
            { code: 'SDD', label: 'SDD — Same Day' },
            { code: 'BULLET', label: 'BULLET — 4 Hours' },
        ];
        // Keep whatever value is already saved selectable even if not in the fallback list.
        if (currentServiceType && !fallback.some(f => f.code === currentServiceType)) {
            fallback.push({ code: currentServiceType, label: escHtml(currentServiceType) });
        }
        serviceOptionsHtml = fallback.map(f =>
            `<option value="${f.code}" ${currentServiceType === f.code ? 'selected' : ''}>${f.label}</option>`
        ).join("\n                ");
    }
...
<body>
    <header class="app-header">
      <a href="/app?shop=${escHtml(shop)}" class="app-header-logo"><span class="app-header-logo-text">PATH<em>X</em>PRESS</span></a>
      <div class="app-header-divider"></div>
      <div class="app-header-shop">
        <span class="app-header-shop-label">Shopify App</span>
        <span class="app-header-shop-name">${escHtml(shop)}</span>
      </div>
      <div class="app-header-right">
        ${shopData && shopData.access_token ? '<span class="badge-connected"><span class="badge-connected-dot"></span>Connected</span>' : '<span class="badge-disconnected"><span class="badge-disconnected-dot"></span>Not connected</span>'}
      </div>
    </header>
    <div class="app-nav-wrap">
      <nav class="app-nav">
        ...
        <a href="/app/stuck-orders?shop=${escHtml(shop)}">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
          Rescue
          ${stuckCountSettings > 0 ? `<span class="nav-badge">${stuckCountSettings}</span>` : ''}
        </a>
    ...
    <main class="app-main">
      ...
      <p class="page-sub">Configure how PathXpress integrates with your Shopify store.</p>
    ...
    <form id="settingsForm">
        ...
        <div class="card">
            ...
            <input type="number" id="clientIdInput" name="clientId" placeholder="Your PathXpress Client ID" value="${escHtml(currentClientId)}" required min="1" class="field" style="font-family:var(--font-mono);" />
            <p class="field-help">Your client ID in the PathXpress portal. Contact support if you don't know it.</p>
        </div>
        ...
        <div class="card">
            ...
            <div id="tagFieldWrapper" style="${currentSyncMode === 'tag' ? '' : 'display:none;'} margin-top:14px;">
                ...
                <p class="field-help">Only orders with this tag will be automatically synced when created.</p>
            </div>
        </div>
        ...
        <div class="card">
            ...
            <div class="grid-2">
                <div>
                    <label class="field-label">Standard (DOM)</label>
                    <input type="number" step="0.01" min="0" name="free_shipping_dom" placeholder="e.g., 200" value="${escHtml(String(freeShippingDOM))}" class="field" style="font-family:var(--font-mono);" />
                    <p class="field-help">1-2 day delivery</p>
                </div>
                <div>
                    <label class="field-label">Express (Same Day)</label>
                    <input type="number" step="0.01" min="0" name="free_shipping_express" placeholder="e.g., 500" value="${escHtml(String(freeShippingExpress))}" class="field" style="font-family:var(--font-mono);" />
                    <p class="field-help">Same day delivery</p>
                </div>
            </div>
        </div>
...
app.post("/api/shipping-rates", async (req, res) => {
    ...
    try {
        ...
        // 5b. DYNAMIC SERVICES: prefer the client's real enabled+priced services
        // from the portal (single source of truth). Scheduled services are skipped
        // because checkout can't capture their per-order date/time. Falls through to
        // the local zone/tier calc below if the portal is unavailable.
        try {
            const freeDOM = shopData.free_shipping_threshold_dom ? parseFloat(shopData.free_shipping_threshold_dom) : 0;
            ...
            if (usable.length) {
                ...
                const rates = usable.map(s => {
                    let price = Number(s.price) || 0;
                    // DOM uses the DOM free-shipping threshold; all express-type services use the Express threshold.
                    const isDom = s.code === 'DOM';
                    const threshold = isDom ? freeDOM : freeExpress;
                    const isFree = threshold > 0 && orderTotal >= threshold;
                    if (isFree) price = 0;
                    const sameDay = s.code === 'SDD' || s.code === 'BULLET' || s.code === 'PREFERRED_TIME_SDD';
                    const minD = sameDay ? now : now + dayMs;
                    const maxD = sameDay ? now + 12 * 60 * 60 * 1000 : now + 2 * dayMs;
                    return {
                        service_name: `PathXpress ${s.displayName}${isFree ? ' (FREE!)' : ''}`,
                        service_code: s.code,
                        total_price: Math.round(price * 100).toString(),
                        currency: "AED",
                        min_delivery_date: new Date(minD).toISOString(),
                        max_delivery_date: new Date(maxD).toISOString(),
                    };
                });
                console.log(`💵 Returning ${rates.length} portal-based rates:`, rates.map(r => `${r.service_code}=${r.total_price}`).join(', '));
                return res.json({ rates });
            }
            console.log("ℹ️ Portal returned no usable services — falling back to local rate calc.");
        } catch (e) {
            console.warn("⚠️ Portal services rate path failed, using local calc:", e.message);
        }
...
<body>
    ...
    <div class="app-nav-wrap">
      <nav class="app-nav">
        ...
        <a href="/app/stuck-orders?shop=${escHtml(shop)}">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
          Rescue
          ${stuckCountOrders > 0 ? `<span class="nav-badge">${stuckCountOrders}</span>` : ''}
        </a>
    ...
    <main class="app-main">
      ...
      <p class="page-sub">Select the orders you want to send to PathXpress.</p>

      <div class="banner-blue" style="width:max-content;max-width:100%;">
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="var(--st-blue)" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
        <span style="font-size:13px;color:var(--ink-2);">Sync mode: <strong style="font-family:var(--font-display);">${escHtml(syncModeLabel)}</strong></span>
        <span style="color:var(--line-strong);">·</span>
        <a href="/app/settings?shop=${escHtml(shop)}" style="color:var(--accent);text-decoration:none;font-weight:600;font-family:var(--font-display);font-size:13px;">Change</a>
      </div>

    <div class="toolbar">
        <button class="btn-primary" id="syncBtn" disabled onclick="syncSelected()"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6"/></svg>Sync Selected (0)</button>
        <label style="font-size:13px;cursor:pointer;display:flex;align-items:center;gap:6px;">
            <input type="checkbox" id="selectAll" onchange="toggleAll(this)"> Select all pending
        </label>
        <div class="search-wrap" style="flex:1;min-width:180px;max-width:280px;margin-left:auto;">
          <svg width="15" height="15" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="11" cy="11" r="7"/><path d="m21 21-4.3-4.3"/></svg>
          <input type="text" id="searchBox" placeholder="Search order or customer…" oninput="filterRows(this.value)">
        </div>
    ...
    <div class="toolbar" style="margin-top:8px;">
        <button class="btn-outline" id="findUnsyncedBtn" onclick="findUnsynced()">
    ...
    <div id="unsyncedSection" style="display:none;margin-top:8px;">
        <div class="section-heading">Unsynced Orders Found</div>
        <div class="card">
            <table>
                <thead>
...
// ======================
// 6.5) RESCUE / STUCK ORDERS PAGE
// ======================
app.get("/app/stuck-orders", requireSessionToken, async (req, res) => {
    const shop = req.shopifySession?.shop || req.query.shop || "";
    ...
    try {
        ...
        // Orders fulfilled but the "delivered" event was never pushed to Shopify.
        // The sync loop records the last event it sent in `last_synced_status`,
        // so a delivered portal order whose last_synced_status isn't "delivered"
        // still needs the delivered event.
        const [noDeliveredRows] = await db.execute(
            `SELECT s.id, s.shop_order_name, s.waybill_number, s.created_at,
                    o.status AS portal_status, o.updatedAt AS last_sync
             FROM shopify_shipments s
             LEFT JOIN orders o ON (o.waybillNumber = s.waybill_number AND o.clientId = ?)
        ...
        const noFulfill = noFulfillRows;
        const noDelivered = noDeliveredRows;

        function renderRows(rows, mode) {
            if (!rows.length) return '';
            return rows.map(r => {
                const ordName = r.shop_order_name || '—';
                const waybill = r.waybill_number || '—';
                const status = r.portal_status || '—';
                const lastSync = r.last_sync ? new Date(r.last_sync).toLocaleDateString() : '—';
                const date = r.created_at ? new Date(r.created_at).toLocaleDateString() : '—';
                return `<tr>
                    <td><strong>${ordName}</strong></td>
                    <td style="font-family:var(--font-mono);font-size:12.5px;">${waybill}</td>
                    <td><span class="badge ${status === 'DELIVERED' ? 'green' : status.includes('TRANSIT') ? 'blue' : 'amber'}">${status}</span></td>
                    <td style="font-size:12.5px;color:var(--muted);">${lastSync}</td>
                    <td style="font-size:12.5px;color:var(--muted);">${date}</td>
                    <td>
                        <button
                            class="btn-sm"
                            onclick="retryRow('${r.id}', '${mode}', this)">
                            Retry
                        </button>
                    </td>
                </tr>`;
        ...
        const NEW_CSS_RESCUE = `
  *,*::before,*::after{box-sizing:border-box}
  body{margin:0;font-family:'Hanken Grotesk',system-ui,sans-serif;background:#F4F3EF;color:#16181C;-webkit-font-smoothing:antialiased;min-height:100vh;padding-bottom:60px;}
  :root{--font-display:'Outfit',system-ui,sans-serif;--font-body:'Hanken Grotesk',system-ui,sans-serif;--font-mono:'Space Mono',ui-monospace,monospace;--radius:14px;--radius-sm:10px;--radius-pill:999px;--bg:#F4F3EF;--surface:#FFFFFF;--surface-2:#FAF9F6;--ink:#16181C;--ink-2:#3C4046;--muted:#6B6F77;--line:rgba(20,22,26,.11);--line-strong:rgba(20,22,26,.20);--accent:#E10600;--accent-ink:#FFFFFF;--band:#16181C;--band-ink:#F4F3EF;--shadow-sm:0 1px 2px rgba(20,22,26,.05);--shadow:0 2px 6px rgba(20,22,26,.05),0 18px 40px -24px rgba(20,22,26,.22);--st-green:#1F8A5B;--st-green-bg:#1f8a5b1a;--st-amber:#B5790A;--st-amber-bg:#b5790a1a;--st-blue:#2A6FDB;--st-blue-bg:#2a6fdb1a;}
  @keyframes pxpulse{0%,100%{opacity:1;transform:scale(1)}50%{opacity:.5;transform:scale(1.3)}}
  .app-header{display:flex;align-items:center;gap:18px;padding:16px 32px;background:var(--surface);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:20;}
  .app-header-logo{display:flex;align-items:center;text-decoration:none;flex:none;}
  .app-header-logo-text{font-family:var(--font-display);font-weight:800;font-size:18px;letter-spacing:-.03em;color:var(--ink);}
  ...
  .nav-badge{font-family:var(--font-mono);font-size:10px;background:var(--accent);color:#fff;border-radius:20px;padding:1px 7px;font-weight:700;}
  .app-main{max-width:1180px;margin:0 auto;padding:26px 32px 0;}
  .page-title{font-family:var(--font-display);font-weight:600;font-size:30px;letter-spacing:-.02em;margin:0 0 4px;}
  p.page-sub{color:var(--muted);font-size:15px;margin:0 0 22px;}
  .card{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow-sm);padding:22px;margin-bottom:18px;}
  .card-nop{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow-sm);margin-bottom:18px;overflow:hidden;}
  .card-header{display:flex;align-items:center;justify-content:space-between;padding:18px 22px;border-bottom:1px solid var(--line);gap:14px;}
  .card-header-title{font-family:var(--font-display);font-weight:600;font-size:16.5px;margin:0;}
  .card-header-sub{font-size:13px;color:var(--muted);margin-top:2px;}
  .table-wrap{overflow-x:auto;padding:8px 8px 6px;}
  table{width:100%;border-collapse:collapse;font-size:13.5px;}
  thead th{text-align:left;font-family:var(--font-mono);font-size:10.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);font-weight:400;padding:8px 14px 12px;border-bottom:1px solid var(--line);}
  tbody td{padding:14px;border-bottom:1px solid var(--line);}
  tbody tr:last-child td{border-bottom:none;}
  .badge{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;font-family:var(--font-display);}
  .badge.green{color:var(--st-green);background:var(--st-green-bg);}
  .badge.amber{color:var(--st-amber);background:var(--st-amber-bg);}
  .badge.blue{color:var(--st-blue);background:var(--st-blue-bg);}
  .badge.red{color:var(--accent);background:color-mix(in srgb,var(--accent) 12%,transparent);}
  ...
  .toolbar{display:flex;align-items:center;gap:14px;flex-wrap:wrap;padding:14px 18px;background:var(--surface);border:1px solid var(--line);border-radius:var(--radius-sm);margin-bottom:16px;box-shadow:var(--shadow-sm);}
  .banner-red{display:flex;align-items:center;gap:14px;padding:16px 18px;border-radius:var(--radius);border:1px solid color-mix(in srgb,var(--accent) 26%,transparent);background:color-mix(in srgb,var(--accent) 6%,transparent);margin-bottom:18px;}
  #log{background:var(--surface-2);border:1px solid var(--line);border-radius:8px;padding:14px;font-family:var(--font-mono);font-size:12px;max-height:250px;overflow-y:auto;margin-top:16px;display:none;}
  .log-line{padding:3px 0;border-bottom:1px solid var(--line);}
  .ok{color:var(--st-green);} .err{color:var(--accent);} .info{color:var(--st-blue);}
  @media(max-width:768px){.app-header{padding:12px 16px;}.app-nav-wrap{padding:12px 16px 0;}.app-main{padding:18px 16px 0;}}
`;
...
<head>
    <meta charset="utf-8"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Rescue Orders — PathXpress</title>
    <meta name="shopify-api-key" content="${process.env.SHOPIFY_API_KEY}"/>
    <script src="https://cdn.shopify.com/shopifycloud/app-bridge.js"></script>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    <link href="https://fonts.googleapis.com/css2?family=Outfit:wght@300;400;500;600;700;800&family=Hanken+Grotesk:wght@400;500;600;700&family=Space+Mono:wght@400;700&display=swap" rel="stylesheet">
    <style>${NEW_CSS_RESCUE}</style>
</head>
<body>
    <header class="app-header">
      <a href="/app?shop=${shop}" class="app-header-logo"><span class="app-header-logo-text">PATH<em>X</em>PRESS</span></a>
      ...
      <div class="app-header-right">
        <span class="badge-connected"><span class="badge-connected-dot"></span>Connected</span>
      </div>
    </header>
    <div class="app-nav-wrap">
      <nav class="app-nav">
        <a href="/app?shop=${shop}">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><rect x="3" y="3" width="7" height="9" rx="1.5"/><rect x="14" y="3" width="7" height="5" rx="1.5"/><rect x="14" y="12" width="7" height="9" rx="1.5"/><rect x="3" y="16" width="7" height="5" rx="1.5"/></svg>
          Dashboard
        </a>
        <a href="/app/orders?shop=${shop}">
        ...
        <a href="/app/stuck-orders?shop=${shop}" class="active">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="9"/><path d="M12 8v4M12 16h.01"/></svg>
          Rescue
          ${(noFulfill.length + noDelivered.length) > 0 ? `<span class="nav-badge">${noFulfill.length + noDelivered.length}</span>` : ''}
        </a>
        <a href="/app/settings?shop=${shop}">
          <svg width="17" height="17" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><circle cx="12" cy="12" r="3"/><path d="M19.4 15a1.65 1.65 0 0 0 .33 1.82l.06.06a2 2 0 1 1-2.83 2.83l-.06-.06a1.65 1.65 0 0 0-1.82-.33 1.65 1.65 0 0 0-1 1.51V21a2 2 0 0 1-4 0v-.09A1.65 1.65 0 0 0 9 19.4a1.65 1.65 0 0 0-1.82.33l-.06.06a2 2 0 1 1-2.83-2.83l.06-.06A1.65 1.65 0 0 0 4.6 15a1.65 1.65 0 0 0-1.51-1H3a2 2 0 0 1 0-4h.09A1.65 1.65 0 0 0 4.6 9a1.65 1.65 0 0 0-.33-1.82l-.06-.06a2 2 0 1 1 2.83-2.83l.06.06A1.65 1.65 0 0 0 9 4.68a1.65 1.65 0 0 0 1-1.51V3a2 2 0 0 1 4 0v.09a1.65 1.65 0 0 0 1 1.51 1.65 1.65 0 0 0 1.82-.33l.06-.06a2 2 0 1 1 2.83 2.83l-.06.06A1.65 1.65 0 0 0 19.4 9a1.65 1.65 0 0 0 1.51 1H21a2 2 0 0 1 0 4h-.09a1.65 1.65 0 0 0-1.51 1z"/></svg>
          Settings
    ...
    <main class="app-main">
      <h1 class="page-title">Rescue orders</h1>
      <p class="page-sub">Orders that could not be synced to Shopify. Retry individually or force all at once.</p>

      <div class="banner-red">
        <svg width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="var(--accent)" stroke-width="1.8" style="flex:none;"><path d="M10.3 3.9 2 18a2 2 0 0 0 1.7 3h16.6a2 2 0 0 0 1.7-3L13.7 3.9a2 2 0 0 0-3.4 0Z"/><path d="M12 9v4M12 17h.01"/></svg>
        <div style="font-size:13.5px;"><strong style="font-family:var(--font-display);">${noFulfill.length + noDelivered.length} orders</strong> need attention and could not be automatically synced.</div>
      </div>

      <div class="toolbar">
        <button class="btn-primary" id="btnAll" onclick="rescueAll()">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M21 12a9 9 0 1 1-2.64-6.36M21 3v6h-6"/></svg>
          Force ALL (fulfillment + delivered)
        </button>
        <button class="btn-outline" id="btnDelivered" onclick="rescueDeliveredOnly()">
          <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M20 6 9 17l-5-5"/></svg>
          Send "delivered" only
        </button>
        <a href="/app?shop=${shop}" style="margin-left:auto;color:var(--muted);font-size:13px;text-decoration:none;font-family:var(--font-display);font-weight:600;">← Back to dashboard</a>
      </div>

      <div class="card-nop" style="margin-bottom:18px;">
        <div class="card-header">
          <div><h3 class="card-header-title">Without Shopify fulfillment</h3><div class="card-header-sub">${noFulfill.length} orders pending</div></div>
          <span class="badge amber">${noFulfill.length}</span>
        </div>
        ${noFulfill.length > 0 ? `
        <div class="table-wrap"><table>
          <thead><tr><th>Order</th><th>Waybill</th><th>Portal status</th><th>Last sync</th><th>Date</th><th></th></tr></thead>
          <tbody>${renderRows(noFulfill, 'fulfill')}</tbody>
        </table></div>` : '<p style="padding:20px 22px;color:var(--muted);font-size:13.5px;">✓ No orders missing fulfillment</p>'}
      </div>

      <div class="card-nop">
        <div class="card-header">
          <div><h3 class="card-header-title">Fulfilled but "Delivered" not sent</h3><div class="card-header-sub">${noDelivered.length} orders pending</div></div>
          <span class="badge red">${noDelivered.length}</span>
        </div>
        ${noDelivered.length > 0 ? `
        <div class="table-wrap"><table>
          <thead><tr><th>Order</th><th>Waybill</th><th>Portal status</th><th>Last sync</th><th>Date</th><th></th></tr></thead>
          <tbody>${renderRows(noDelivered, 'delivered')}</tbody>
        </table></div>` : '<p style="padding:20px 22px;color:var(--muted);font-size:13.5px;">✓ No orders missing delivered event</p>'}
      </div>
    ...
    <script>
        ...
        async function retryRow(id, mode, btn) {
            btn.disabled = true;
            btn.textContent = '...';
            log('Retrying ' + mode + ' for ID ' + id + '...', 'info');
            try {
                const token = await getToken();
                const headers = { 'Content-Type': 'application/json' };
                if (token) headers['Authorization'] = 'Bearer ' + token;
                const r = await fetch('/api/rescue-order', {
                    method: 'POST',
                    headers,
                    body: JSON.stringify({ shop: SHOP, id, mode })
                });
                const d = await r.json();
                if (d.success) {
                    log('✓ Done: ' + (d.message || 'OK'), 'ok');
        ...
        async function rescueAll() {
            ...
            btn.disabled = true;
            log('Starting force-all rescue...', 'info');
            try {
                const token = await getToken();
                const headers = { 'Content-Type': 'application/json' };
                if (token) headers['Authorization'] = 'Bearer ' + token;
                const r = await fetch('/api/rescue-order', {
                    method: 'POST',
                    headers,
                    body: JSON.stringify({ shop: SHOP, mode: 'all' })
                });
                const d = await r.json();
                log(d.success ? ('✓ ' + (d.message || 'Done')) : ('✗ ' + d.error), d.success ? 'ok' : 'err');
            } catch(e) {
                log('Network error: ' + e.message, 'err');
            }
            btn.disabled = false;
        ...
        async function rescueDeliveredOnly() {
            var btn = document.getElementById('btnDelivered');
            btn.disabled = true;
            log('Sending delivered events only...', 'info');
            try {
                const token = await getToken();
                const headers = { 'Content-Type': 'application/json' };
                if (token) headers['Authorization'] = 'Bearer ' + token;
                const r = await fetch('/api/rescue-order', {
                    method: 'POST',
                    headers,
                    body: JSON.stringify({ shop: SHOP, mode: 'delivered' })
                });
                const d = await r.json();
                log(d.success ? ('✓ ' + (d.message || 'Done')) : ('✗ ' + d.error), d.success ? 'ok' : 'err');
            } catch(e) {
        ...
        console.error("Error loading rescue page:", err);
        res.status(500).send("Error loading rescue page: " + err.message);
    }
});

// --- Rescue action API endpoint ---
app.post("/api/rescue-order", requireSessionToken, express.json(), async (req, res) => {
    const shop = req.shopifySession?.shop || req.body?.shop || "";
    ...
    try {
        const shopData = await getShopFromDB(shop);
        if (!shopData || !shopData.access_token) {
            return res.json({ success: false, error: "Shop not connected" });
        }

        if (mode === 'all' || mode === 'delivered') {
            // Batch operations — trigger existing sync logic
            await syncShipmentsToShopify();
            return res.json({ success: true, message: "Sync triggered. Check back in a moment." });
        }

        if (id && mode === 'fulfill') {
            // Retry fulfillment for a single shipment
            const [rows] = await db.execute(
                "SELECT * FROM shopify_shipments WHERE id = ? AND shop_domain = ?",
                [id, shop]
            );
            if (!rows.length) return res.json({ success: false, error: "Shipment not found" });
            await syncShipmentsToShopify();
            return res.json({ success: true, message: "Fulfillment sync triggered" });
        }

        return res.json({ success: false, error: "Unknown mode" });
    } catch(err) {
...
// ======================
// 6.6) TRACKING TIMELINE + POD (read-only, from shared DB)
// ======================
app.get("/app/orders/:waybill/tracking", requireSessionToken, async (req, res) => {
    const shop = req.shopifySession?.shop || req.query.shop || "";
    ...
    try {
        const shopData = await getShopFromDB(shop);
        const clientId = shopData?.pathxpress_client_id || 0;

        // Resolve the order by waybill, scoped to this shop's client (tenant isolation)
        const [orderRows] = await db.execute(
            `SELECT id, orderNumber, waybillNumber, status, serviceType, customerName,
                    city, destinationCountry, codRequired, codAmount, codCurrency,
                    createdAt, lastStatusUpdate
             FROM orders WHERE waybillNumber = ? AND clientId = ? LIMIT 1`,
            [waybill, clientId]
        );
        ...
        if (order) {
            const [evRows] = await db.execute(
                `SELECT eventDatetime, location, statusCode, statusLabel, description,
                        podFileUrl, podFileUrl2, createdAt
                 FROM trackingEvents WHERE shipmentId = ?
        ...
        const badgeClass = (st) => {
            ...
            if (s.includes('TRANSIT') || s.includes('PICKED') || s.includes('OUT_FOR')) return 'blue';
            if (s.includes('FAIL') || s.includes('RETURN') || s.includes('CANCEL')) return 'red';
            return 'amber';
        };
        ...
        const timelineHtml = events.length ? events.map((ev, i) => {
            const pods = [ev.podFileUrl, ev.podFileUrl2].filter(Boolean);
            const podHtml = pods.length ? `
                <div style="display:flex;gap:10px;margin-top:10px;flex-wrap:wrap;">
                  ${pods.map(u => `<a href="${escHtml(u)}" target="_blank" rel="noopener" style="display:block;"><img src="${escHtml(u)}" alt="Proof of delivery" style="width:96px;height:96px;object-fit:cover;border-radius:10px;border:1px solid var(--line);"></a>`).join('')}
                </div>` : '';
            return `
              <div style="position:relative;padding:0 0 22px 28px;">
                <span style="position:absolute;left:0;top:3px;width:13px;height:13px;border-radius:50%;background:var(--st-${badgeClass(ev.statusCode || ev.statusLabel)},var(--accent));box-shadow:0 0 0 3px var(--surface);"></span>
                ${i < events.length - 1 ? '<span style="position:absolute;left:6px;top:16px;bottom:0;width:1px;background:var(--line);"></span>' : ''}
                <div style="display:flex;align-items:center;gap:10px;flex-wrap:wrap;">
                  <span class="badge ${badgeClass(ev.statusCode || ev.statusLabel)}">${escHtml(ev.statusLabel || ev.statusCode || '—')}</span>
                  <span style="font-size:12.5px;color:var(--muted);font-family:var(--font-mono);">${escHtml(fmt(ev.eventDatetime))}</span>
                </div>
                ${ev.location ? `<div style="font-size:13px;color:var(--ink-2);margin-top:6px;">📍 ${escHtml(ev.location)}</div>` : ''}
                ${ev.description ? `<div style="font-size:13px;color:var(--muted);margin-top:4px;">${escHtml(ev.description)}</div>` : ''}
                ${podHtml}
              </div>`;
        }).join('') : '<p style="color:var(--muted);font-size:14px;padding:8px 0;">No tracking events yet for this shipment.</p>';

        const css = `
  *,*::before,*::after{box-sizing:border-box} body{margin:0;font-family:'Hanken Grotesk',system-ui,sans-serif;background:#F4F3EF;color:#16181C;-webkit-font-smoothing:antialiased;min-height:100vh;padding-bottom:60px;}
  :root{--font-display:'Outfit',system-ui,sans-serif;--font-body:'Hanken Grotesk',system-ui,sans-serif;--font-mono:'Space Mono',ui-monospace,monospace;--radius:14px;--radius-sm:10px;--radius-pill:999px;--bg:#F4F3EF;--surface:#FFFFFF;--surface-2:#FAF9F6;--ink:#16181C;--ink-2:#3C4046;--muted:#6B6F77;--line:rgba(20,22,26,.11);--line-strong:rgba(20,22,26,.20);--accent:#E10600;--accent-ink:#FFFFFF;--st-green:#1F8A5B;--st-green-bg:#1f8a5b1a;--st-amber:#B5790A;--st-amber-bg:#b5790a1a;--st-blue:#2A6FDB;--st-blue-bg:#2a6fdb1a;--st-red:#E10600;--st-red-bg:#e106001a;--shadow-sm:0 1px 2px rgba(20,22,26,.05);}
  .app-header{display:flex;align-items:center;gap:18px;padding:16px 32px;background:var(--surface);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:20;}
  .app-header-logo-text{font-family:var(--font-display);font-weight:800;font-size:18px;letter-spacing:-.03em;color:var(--ink);text-decoration:none;}
  .app-header-logo-text em{color:var(--accent);font-style:normal;}
  .app-main{max-width:840px;margin:0 auto;padding:26px 32px 0;}
  .page-title{font-family:var(--font-display);font-weight:600;font-size:28px;letter-spacing:-.02em;margin:0 0 4px;}
  p.page-sub{color:var(--muted);font-size:15px;margin:0 0 22px;}
  .card{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow-sm);padding:22px;margin-bottom:18px;}
  .badge{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;font-family:var(--font-display);}
  .badge.green{color:var(--st-green);background:var(--st-green-bg);} .badge.amber{color:var(--st-amber);background:var(--st-amber-bg);}
...
<head>
    <meta charset="utf-8"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>Tracking ${escHtml(waybill)} — PathXpress</title>
    <meta name="shopify-api-key" content="${process.env.SHOPIFY_API_KEY}"/>
    <script src="https://cdn.shopify.com/shopifycloud/app-bridge.js"></script>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
...
<body>
    ...
    <main class="app-main">
      ...
      <p class="page-sub">Waybill <strong style="font-family:var(--font-mono);">${escHtml(order.waybillNumber)}</strong>${order.orderNumber ? ` · Order ${escHtml(order.orderNumber)}` : ''}</p>

      <div class="card">
        <div class="sumrow"><span>Current status</span><span><span class="badge ${badgeClass(order.status)}">${escHtml(String(order.status || '').toUpperCase())}</span></span></div>
        <div class="sumrow"><span>Service</span><span>${escHtml(order.serviceType || '—')}</span></div>
        <div class="sumrow"><span>Recipient</span><span>${escHtml(order.customerName || '—')}</span></div>
        <div class="sumrow"><span>Destination</span><span>${escHtml(order.city || '')}${order.destinationCountry ? `, ${escHtml(order.destinationCountry)}` : ''}</span></div>
        ${order.codRequired && order.codAmount ? `<div class="sumrow"><span>COD</span><span>${escHtml(order.codCurrency || 'AED')} ${escHtml(parseFloat(order.codAmount).toFixed(2))}</span></div>` : ''}
        <div class="sumrow"><span>Created</span><span>${escHtml(fmt(order.createdAt))}</span></div>
      </div>

      <div class="card">
        <h3 style="font-family:var(--font-display);font-weight:600;font-size:16px;margin:0 0 18px;">Tracking history</h3>
        ${timelineHtml}
      </div>
      `}
    </main>
        ...
        res.status(500).send("Error loading tracking: " + err.message);
    }
});

// ======================
// 6.7) COD RECONCILIATION (read-only, from shared DB)
// ======================
app.get("/app/cod", requireSessionToken, async (req, res) => {
    const shop = req.shopifySession?.shop || req.query.shop || "";
    if (!shop) return res.status(400).send("Missing shop.");
    ...
    try {
        ...
        // COD records for this shop's shipments only (join through shopify_shipments).
        const [rows] = await db.execute(
            `SELECT cr.codAmount, cr.codCurrency, cr.status,
                    cr.collectedDate, cr.remittedToClientDate,
                    o.orderNumber, o.waybillNumber, o.customerName, o.createdAt
             FROM codRecords cr
             JOIN orders o ON o.id = cr.shipmentId
             JOIN shopify_shipments s ON (s.waybill_number = o.waybillNumber AND s.shop_domain = ?)
             WHERE o.clientId = ?
             ORDER BY o.createdAt DESC
             LIMIT 300`,
            [shop, clientId]
        ...
        let pendingTotal = 0, collectedTotal = 0, remittedTotal = 0;
        let pendingCount = 0, collectedCount = 0, remittedCount = 0;
        for (const r of rows) {
            const amt = num(r.codAmount);
            if (r.status === 'pending_collection') { pendingTotal += amt; pendingCount++; }
            else if (r.status === 'collected') { collectedTotal += amt; collectedCount++; }
            else if (r.status === 'remitted') { remittedTotal += amt; remittedCount++; }
        }
        const money = n => n.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 });

        const statusBadge = (st) => {
            switch (st) {
                case 'collected': return '<span class="badge blue">Collected</span>';
                case 'remitted': return '<span class="badge green">Remitted</span>';
                case 'disputed': return '<span class="badge red">Disputed</span>';
                case 'cancelled': return '<span class="badge gray">Cancelled</span>';
                default: return '<span class="badge amber">Pending</span>';
            }
        };
        ...
        const rowsHtml = rows.length ? rows.map(r => `
            <tr>
              <td><strong>${escHtml(r.orderNumber || '—')}</strong></td>
              <td style="font-family:var(--font-mono);font-size:12.5px;">${escHtml(r.waybillNumber || '—')}</td>
              <td>${escHtml(r.customerName || '—')}</td>
              <td style="font-family:var(--font-display);font-weight:600;">${escHtml(r.codCurrency || 'AED')} ${escHtml(money(num(r.codAmount)))}</td>
              <td>${statusBadge(r.status)}</td>
              <td style="font-size:12.5px;color:var(--muted);">${escHtml(fmtDate(r.collectedDate))}</td>
              <td style="font-size:12.5px;color:var(--muted);">${escHtml(fmtDate(r.remittedToClientDate))}</td>
            </tr>`).join('') : '<tr><td colspan="7" style="padding:22px;text-align:center;color:var(--muted);">No COD records found for this store.</td></tr>';

        const css = `
  *,*::before,*::after{box-sizing:border-box} body{margin:0;font-family:'Hanken Grotesk',system-ui,sans-serif;background:#F4F3EF;color:#16181C;-webkit-font-smoothing:antialiased;min-height:100vh;padding-bottom:60px;}
  :root{--font-display:'Outfit',system-ui,sans-serif;--font-body:'Hanken Grotesk',system-ui,sans-serif;--font-mono:'Space Mono',ui-monospace,monospace;--radius:14px;--radius-sm:10px;--radius-pill:999px;--bg:#F4F3EF;--surface:#FFFFFF;--surface-2:#FAF9F6;--ink:#16181C;--ink-2:#3C4046;--muted:#6B6F77;--line:rgba(20,22,26,.11);--line-strong:rgba(20,22,26,.20);--accent:#E10600;--band:#16181C;--band-ink:#F4F3EF;--st-green:#1F8A5B;--st-green-bg:#1f8a5b1a;--st-amber:#B5790A;--st-amber-bg:#b5790a1a;--st-blue:#2A6FDB;--st-blue-bg:#2a6fdb1a;--st-red:#E10600;--st-red-bg:#e106001a;--shadow-sm:0 1px 2px rgba(20,22,26,.05);}
  .app-header{display:flex;align-items:center;gap:18px;padding:16px 32px;background:var(--surface);border-bottom:1px solid var(--line);position:sticky;top:0;z-index:20;}
  .app-header-logo-text{font-family:var(--font-display);font-weight:800;font-size:18px;letter-spacing:-.03em;color:var(--ink);text-decoration:none;}
  .app-header-logo-text em{color:var(--accent);font-style:normal;}
  ...
  .kpi-label{font-size:13px;color:var(--muted);font-weight:500;} .kpi-card.band .kpi-label{color:color-mix(in srgb,var(--band-ink) 65%,transparent);}
  .kpi-val{font-family:var(--font-display);font-weight:600;font-size:30px;letter-spacing:-.03em;margin:12px 0 4px;}
  .kpi-sub{font-size:12.5px;color:var(--muted);} .kpi-card.band .kpi-sub{color:color-mix(in srgb,var(--band-ink) 65%,transparent);}
  .card-nop{background:var(--surface);border:1px solid var(--line);border-radius:var(--radius);box-shadow:var(--shadow-sm);overflow:hidden;}
  .card-header{padding:18px 22px;border-bottom:1px solid var(--line);} .card-header-title{font-family:var(--font-display);font-weight:600;font-size:16.5px;margin:0;}
  .table-wrap{overflow-x:auto;padding:8px 8px 6px;} table{width:100%;border-collapse:collapse;font-size:13.5px;}
  thead th{text-align:left;font-family:var(--font-mono);font-size:10.5px;letter-spacing:.1em;text-transform:uppercase;color:var(--muted);font-weight:400;padding:8px 14px 12px;border-bottom:1px solid var(--line);white-space:nowrap;}
  tbody td{padding:14px;border-bottom:1px solid var(--line);} tbody tr:last-child td{border-bottom:none;}
  .badge{display:inline-flex;align-items:center;gap:6px;padding:4px 10px;border-radius:20px;font-size:12px;font-weight:600;font-family:var(--font-display);}
  .badge.green{color:var(--st-green);background:var(--st-green-bg);} .badge.amber{color:var(--st-amber);background:var(--st-amber-bg);}
  .badge.blue{color:var(--st-blue);background:var(--st-blue-bg);} .badge.red{color:var(--accent);background:var(--st-red-bg);} .badge.gray{color:var(--muted);background:color-mix(in srgb,var(--muted) 14%,transparent);}
  @media(max-width:768px){.app-header{padding:12px 16px;}.app-main{padding:18px 16px 0;}.kpi-grid{grid-template-columns:1fr;}}`;
...
<head>
    <meta charset="utf-8"/>
    <meta name="viewport" content="width=device-width, initial-scale=1.0"/>
    <title>COD Reconciliation — PathXpress</title>
    <meta name="shopify-api-key" content="${process.env.SHOPIFY_API_KEY}"/>
    <script src="https://cdn.shopify.com/shopifycloud/app-bridge.js"></script>
    <link rel="preconnect" href="https://fonts.googleapis.com">
    <link rel="preconnect" href="https://fonts.gstatic.com" crossorigin>
    ...
    <main class="app-main">
      ...
      <div class="kpi-grid">
        <div class="kpi-card">
          ...
          <div class="kpi-val">AED ${money(pendingTotal)}</div>
          <div class="kpi-sub">${pendingCount} shipment${pendingCount === 1 ? '' : 's'} out for COD</div>
        </div>
        <div class="kpi-card">
          <div class="kpi-label">Collected (to remit)</div>
          <div class="kpi-val">AED ${money(collectedTotal)}</div>
          <div class="kpi-sub">${collectedCount} collected, awaiting remittance</div>
        </div>
        <div class="kpi-card band">
          <div class="kpi-label">Remitted to you</div>
          <div class="kpi-val">AED ${money(remittedTotal)}</div>
          <div class="kpi-sub">${remittedCount} settled</div>
        </div>
      </div>

      <div class="card-nop">
        <div class="card-header"><h3 class="card-header-title">COD records</h3></div>
        <div class="table-wrap">
          <table>
            <thead><tr><th>Order</th><th>Waybill</th><th>Customer</th><th>Amount</th><th>Status</th><th>Collected</th><th>Remitted</th></tr></thead>
        ...
        console.error("Error loading COD page:", err);
        res.status(500).send("Error loading COD: " + err.message);
    }
});

// ======================
// 6.8) WAYBILL PDF PROXY → portal (keeps the integration secret server-side)
// ======================
app.get("/app/waybill/:waybill.pdf", requireSessionToken, async (req, res) => {
    const shop = req.shopifySession?.shop || req.query.shop || "";
    const waybill = req.params.waybill || "";
    if (!shop || !waybill) return res.status(400).send("Missing shop or waybill.");
    ...
    if (!INTEGRATION_SECRET) return res.status(503).send("Waybill service not configured.");
    ...
    try {
        ...
        const url = `${PORTAL_URL}/api/shopify/waybill/${encodeURIComponent(waybill)}.pdf?${params.toString()}`;
        const r = await fetch(url, { headers: { "Authorization": `Bearer ${INTEGRATION_SECRET}` } });

        if (!r.ok) {
            console.warn(`⚠️ Portal waybill ${waybill} returned ${r.status}`);
            return res.status(r.status === 404 ? 404 : 502).send("Could not fetch waybill from portal.");
        }

        const buf = Buffer.from(await r.arrayBuffer());
        res.setHeader("Content-Type", "application/pdf");
        res.setHeader("Content-Disposition", `inline; filename="waybill-${waybill}.pdf"`);
        res.send(buf);
    } catch (err) {
        console.error("⛔ Error proxying waybill PDF:", err);
        res.status(500).send("Error fetching waybill: " + err.message);
    }
});
...
// ======================
// PORTAL: obtener servicios disponibles para un cliente
// Reutiliza la misma URL + Bearer secret que sendShipmentToPathxpress.
//   clientId requerido. emirate + weight opcionales → activan precios/disponibilidad.
// Devuelve [] si falla (el caller debe tener fallback).
// ======================
async function fetchPortalServices(clientId, { emirate, weight } = {}) {
    const PORTAL_URL = process.env.PATHXPRESS_PORTAL_URL || "https://pathxpress.net";
    const INTEGRATION_SECRET = process.env.SHOPIFY_INTEGRATION_SECRET;

    if (!INTEGRATION_SECRET || !clientId) return [];

    const params = new URLSearchParams({ clientId: String(clientId) });
    if (emirate) params.set("emirate", String(emirate));
    if (weight) params.set("weight", String(weight));

    try {
        const response = await fetch(`${PORTAL_URL}/api/shopify/services?${params.toString()}`, {
            headers: { "Authorization": `Bearer ${INTEGRATION_SECRET}` },
        });
        if (!response.ok) {
            console.warn("⚠️ Portal /services no OK:", response.status);
            return [];
        }
        const json = await response.json();
